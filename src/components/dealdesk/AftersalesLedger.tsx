/**
 * AftersalesLedger — the former "Deal Ledger" (AdminAftersales) page body,
 * extracted so it can be embedded as tabs inside the Deal Desk.
 *
 * NOTHING here changes the profit math: the month-grouped table, the current-
 * month performance bar and the per-rep commission boards all read the SAME
 * authoritative `gross_profit` column written at Finalize time. We only moved
 * the JSX/logic out of the page wrapper (AdminLayout/Helmet) and added a `view`
 * prop so the host can show the profit/ledger half and the customer-follow-ups
 * half on separate tabs.
 *
 *   view="ledger"    → performance bar + commission boards + month-grouped
 *                      profit table + all deal-management actions
 *                      (lock/unlock, edit, undo, partner PDF, manage drawer).
 *   view="followups" → the aftersales_records "Customer Follow-ups" table.
 */
...
export type AftersalesLedgerView = 'ledger' | 'followups';

interface AftersalesLedgerProps {
  /** Which half of the old Deal Ledger to render. */
  view: AftersalesLedgerView;
}

/**
 * The Deal Ledger content, minus the page chrome. Hosted by AdminDealDesk
 * (and by the legacy AdminAftersales page, which now just redirects). All deal
 * math is unchanged — see file header.
 */
const AftersalesLedger = ({ view }: AftersalesLedgerProps) => {
  const queryClient = useQueryClient();
    ...
    <div className="space-y-6">
      ...
      {zeroCostDealsCount > 0 && (
        <motion.div
          ...
          className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg flex items-center gap-3"
        >
...
export default AftersalesLedger;
