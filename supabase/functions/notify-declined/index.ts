...
serve(async (req) => {
  ...
  try {
    ...
    if (SU && SK) {
      const gate = await fetch(`${SU}/rest/v1/whatsapp_templates?key=eq.declined&select=active`, { headers: { apikey: SK, Authorization: `Bearer ${SK}` } });
  ...
  try {
    ...
    for (const token of tokens) {
      ...
      console.log("EasySocial Response (declined):", responseData);
