...
type PartyRow   = { page?: number; y_start: number; y_step: number; name_x: number; id_x: number; role_x?: number; share_x: number; size: number; max_rows: number; };
type EntityKey = "company_name" | "trading_name" | "registration_number" | "entity_type" | "tax_number" | "vat_number" | "nature_of_business" | "years_in_business" | "registered_address" | "postal_address" | "registered_office" | "contact_phone" | "contact_email" | "banker" | "branch" | "branch_code" | "account_number" | "account_type" | "auditor_name" | "auditor_phone" | "turnover" | "signer_full_name" | "signer_capacity" | "signature_date" | "signed_at";
type BankMap = { fields: Partial<Record<EntityKey, FieldCoord>>; parties?: PartyRow; signature: SigCoord; };
...
const standardBankMap: BankMap = {
  ...
  parties: { y_start: 320, y_step: -15, name_x: 310, id_x: 420, share_x: 530, size: 8, max_rows: 6 },
...
const absaMap: BankMap = {
  ...
  parties: { page: 0, y_start: 385, y_step: -25, name_x: 60, id_x: 230, role_x: 400, share_x: 500, size: 9, max_rows: 6 },
...
const mfcMap: BankMap = {
  ...
  parties: { y_start: 550, y_step: -25, name_x: 80, id_x: 280, share_x: 470, size: 9, max_rows: 5 },
...
const wesbankMap: BankMap = {
  fields: {
    ...
    signed_at:           { x: 400, y: 160, size: 10 },
  },
  parties: { y_start: 450, y_step: -25, name_x: 80, id_x: 320, role_x: 500, share_x: 680, size: 9, max_rows: 6 },
  signature: { x: 160, y: 100, width: 120, height: 40 },
};

const BANK_MAPS: Record<Exclude<Bank, "fic">, BankMap> = { juristic: standardBankMap, absa: absaMap, mfc_ubo: mfcMap, wesbank: wesbankMap };
const s = (v: unknown): string => (v === null || v === undefined ? "" : String(v));

async function fetchSignature(url: string | null): Promise<Uint8Array | null> {
  if (!url) return null;
  try {
    if (url.startsWith("data:")) { return Uint8Array.from(atob(url.split(",")[1]), c => c.charCodeAt(0)); }
...
async function drawSignatureScaled(pdf: PDFDocument, page: any, bytes: Uint8Array | null, coord: SigCoord) {
  ...
  try {
    ...
    page.drawImage(img, { x: coord.x, y: coord.y, width: coord.width, height: coord.height });
...
async function fillFlatPdf(tplBytes: Uint8Array, bank: Exclude<Bank, "fic">, row: Record<string, any>, sigBytes: Uint8Array | null): Promise<Uint8Array> {
  ...
  for (const [key, coord] of Object.entries(map.fields) as [EntityKey, FieldCoord][]) {
    ...
    if (!page) continue;
    page.drawText(entityValue(row, key), { x: coord.x, y: coord.y, size: coord.size, font, color: rgb(0, 0, 0) });
  }
  ...
  if (map.parties) {
    ...
    parties.slice(0, p.max_rows).forEach((party: any, i: number) => {
      ...
      if (p.role_x !== undefined) { page.drawText(s(party.designation), { x: p.role_x, y, size: p.size, font }); }
...
async function fillFicForm(tplBytes: Uint8Array, row: Record<string, any>, sigBytes: Uint8Array | null): Promise<Uint8Array> {
  const pdf = await PDFDocument.load(tplBytes);
  const form = pdf.getForm();
  const set = (name: string, value: string) => { try { form.getTextField(name).setText(s(value)); } catch { } };
  ...
  parties.slice(0, blocks.length).forEach((p: any, i: number) => {
    ...
    set(`Shareholder Information: 1${sfx}`, firstName);
    set(`Shareholder Information: 2${sfx}`, surname);
    set(`Shareholder Information: 3${sfx}`, p.id_number);
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!submission_id || !bank) return new Response(JSON.stringify({ error: "submission_id and bank required" }), { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    const admin = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);
    const { data: row, error: rowErr } = await admin.from("juristic_submissions").select("*").eq("id", submission_id).maybeSingle();
    if (rowErr || !row) return new Response(JSON.stringify({ error: "submission not found" }), { status: 404, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    const { data: tpl, error: tplErr } = await admin.storage.from("bank-templates").download(`${bank}.pdf`);
    if (tplErr || !tpl) return new Response(JSON.stringify({ error: `template ${bank}.pdf missing` }), { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } });

    const tplBytes = new Uint8Array(await tpl.arrayBuffer());
    let sigBytes: Uint8Array | null = null;
    if (row.signature_image_url) {
      if (row.signature_image_url.startsWith("data:")) { sigBytes = await fetchSignature(row.signature_image_url); }
      else {
    ...
    const out = bank === "fic" ? await fillFicForm(tplBytes, row, sigBytes) : await fillFlatPdf(tplBytes, bank as Exclude<Bank, "fic">, row, sigBytes);
    return new Response(out, { status: 200, headers: { ...corsHeaders, "Content-Type": "application/pdf", "Content-Disposition": `attachment; filename="${row.company_name ?? "juristic"}_${bank}.pdf"` } });
  } catch (e) {
