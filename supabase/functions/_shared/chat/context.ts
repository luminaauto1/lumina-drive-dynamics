// context.ts — FAITHFUL TypeScript port of lumina-chat/engine/context.js.
// Reads the ENTIRE conversation and distils profile / unanswered questions /
// already-sent replies / last bot question. Pure & deterministic. No AI.
...
function norm(s: any): string { return (s || "").toString().toLowerCase().replace(/\s+/g, " ").trim(); }

export const CREDIT_MAP: Record<string, string> = {
...
export const LICENCE_MAP: Record<string, string> = { "yes i do": "yes", "no licence yet": "no", "i only have learners": "learners" };
export const INCOME_MAP: Record<string, string> = { "yes sir!": "yes", "not yet": "no", "it's complicated": "complicated" };
const pick = (map: Record<string, string>, v: any) => (v == null ? null : map[String(v).trim().toLowerCase()] || null);

export interface ChatContext {
  profile: {
    licence: string | null; credit: string | null; income: string | null;
    net_income: any; marital: any; bank: any; name: string;
  };
  tags: string[];
  unanswered: string[];
  effectiveInbound: string;
...
export function buildContext(messages: any[], lead: any, kb: any): ChatContext {
  const sorted = (messages || []).slice().sort((a, b) => new Date(a.created_at || 0).getTime() - new Date(b.created_at || 0).getTime());
  ...
  const tags = (lead && (lead.latest_tags || []).map((t: any) => t.title || t.name || t)) || [];
